...
import { useForm, FormItem, Input, Checkbox, Select, MultiTimeSelect,} from "@app/components/Form"
...
type DayIdsByDayType = Record<number | string,{ starttime: string; endtime: string }>
type TimeDefaultsType = Record<string, { starttime: string; endtime: string }>
const linkToRotationsTipMessage ="Click this checkbox if you want to link this job to rotations in Block Scheduling calendar(s) in your system, and only assign residents who are assigned to specific rotations. You will need to setup the Linked Rotations for this job separately."
...
export default (props: Props) => {
  ...
  const multipleAssignmentsEnabled = useCaniuseFeature("multiple_assignments", {scope: "group",})
  ...
  const [multiTimeDefaults] = useState<TimeDefaultsType>(() => {
    console.log(job?.day_types, "multiTimeDefaults")
    console.log(watchedStartTime, "watchedStartTime")
    return (
  ...
  return (
    ...
    >
      ...
      <Fragment>
        <FormItem label="Default Job Time" required>
          <Row>
            <Col>
